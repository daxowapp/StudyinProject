...
// 5. Replace massive useMemo with useQuery
const regex = /    \/\/ Filter programs based on current filters, then rank by relevance[\s\S]*?    const paginatedPrograms = filteredPrograms.slice\(startIndex, endIndex\);/;
...
if (!regex.test(content)) {
    console.error("Could not find filteredPrograms regex match!");
    process.exit(1);
}
...
content = content.replace(/filteredPrograms\.length/g, "totalCount");
// Stop loading indicator on UI if needed, actually we have isFetchingPrograms
